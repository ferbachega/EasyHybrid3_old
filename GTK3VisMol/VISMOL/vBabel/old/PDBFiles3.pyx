...
def load_pdb_files (infile = None, VMSession =  None):
    ...
    with open(infile, 'r') as pdb_file:
        ...
        list_cov_rad, list_name, list_resi, list_resn, list_chain = get_atom_list_from_pdb_frame(frames[0])
...
def get_atom_list_from_pdb_frame (frame):
    ...
    #'''
        if line[:4] == 'ATOM' or line[:6] == 'HETATM':
            ...
            list_cov_rad.append(cov_rad)
            list_name   .append(at_name)
            list_resi   .append(at_res_i)
            list_resn   .append(at_res_n)
            list_chain  .append(at_ch)
            
            
            
    list_cov_rad = np.array(list_cov_rad, dtype=np.float32)
    
    return list_cov_rad, list_name, list_resi, list_resn, list_chain  #atoms#, coords
