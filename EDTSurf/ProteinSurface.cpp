...
struct point *ProteinSurface::getvertices()
{
	struct point *vs = static_cast<struct point *>(
											malloc(vertnumber * sizeof(*vs)));
	if (vs) {
		for (int i = 0; i < vertnumber; ++i) {
			vs[i].x = verts[i].x/scalefactor - ptran.x;
			vs[i].y = verts[i].y/scalefactor - ptran.y;
			vs[i].z = verts[i].z/scalefactor - ptran.z;
		}
	}
	return vs;
}

struct triangle *ProteinSurface::gettriangles()
{
	struct triangle *ts = static_cast<struct triangle *>(
											malloc(facenumber * sizeof(*ts)));
	if (ts) {
...
void ProteinSurface::freevertices(struct point *vs)
{
	free(vs);
}

void ProteinSurface::freetriangles(struct triangle *ts)
{
	free(ts);
}
